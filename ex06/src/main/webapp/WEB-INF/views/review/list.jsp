                        ...
                        <c:when test="${not empty list}">
                            <c:forEach var="review" items="${list}">
                                <tr class="review-row">
                                    ...
                                    <td>${review.region}</td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/review/get?review_id=${review.review_id}">
                                            ${review.review_title}
                                        </a>
                                    </td>
    ...
    <c:if test="${pageMaker.prev}">
        <c:url var="prevLink" value="/review/list">
            <c:param name="pageNum" value="${pageMaker.startPage - 1}" />
             <c:if test="${not empty cri.amount}"><c:param name="amount" value="${cri.amount }"/></c:if>
            <c:if test="${not empty cri.type}"><c:param name="type" value="${cri.type}" /></c:if>
